# AGI Interpreter for the Tandy Color Computer 3 — design spec v1.3
...
**Changes from v1.2** (2026-09-13) — ★★★★★ **the text subsystem exists end to end: a typed line
reaches `said()` and the room answers it. And four "real hardware" claims are corrected — NOTHING HAS
EVER RUN ON A REAL CoCo3.**

- ★★★★★ **§0.1 NEW — MAME is not hardware.** Every result in this document was measured under MAME.
  **Hardware is a target, not a result**, and v1.1/v1.2 described P3 and P3b as hardware milestones.
  They were not.
- ★★★★★ **NEW §8B — the text subsystem.** Nine opcodes, the message box, the blocking wait, the
  command line, and the interrupt the wait depends on.
- ★★★★★ **§8B.3 — LOGIC decryption is bound to the BIND, not the fetch**, following the oracle at
  `agi.cpp:489-500` / `logic.cpp:56`.
- ★★★★★ **§8B.7 — a typed line reaches `said()`.** `promptKeyPress` is per-cycle, not blocking;
  ENTER parses in place, inside the vocabulary window.
- ★★★★ **§8B.2 — the text window is a PHASE**, four slots, rows 0-24. `mmu_phase.s` is the sole
  writer of the MMU register file again.
- ★★★★ **§8A.4's "what does not exist yet" is SUPERSEDED in part** — keyboard input, text rendering
  and the command line now exist. The `>` prefix and the status line do not.
- ★★★ **§5.4a's figures stand; `vm_timed`'s medians are RETIRED, not restated** — the VM now
  decrypts on a fresh LOGIC bind and does more work than P6.10's numbers describe.
...
- ★★★ **§6.5 NEW — first pixels, gated.** One AGI PICTURE interpreted by 6809 code **under MAME** in
...
## 0.1 ★★★★★ MAME is not hardware, and nothing has run on hardware yet

> ★★★★★ **EVERY RESULT IN THIS DOCUMENT WAS MEASURED UNDER MAME. As of 2026-09-13 the port has never
> run on a real CoCo 3.**

★★★★ **v1.1 and v1.2 described P3 and P3b as milestones "on real hardware". They were not**, and the
wording is corrected throughout this revision. ★★★ The phrase appeared four times and twice carried
no qualifier at all, which is how a reader — and a dispatch written from it — would have taken it as
a result.

**The words, and one meaning each:**

| word | means |
|---|---|
| **oracle** | ScummVM pinned at `9d9b9e93` |
| **reference** | `tools/agivm/`, the offline Python that tracks it |
| **port** | our 6809 code — the artefact this document specifies |
| **host** | `p3b_run.lua`, `p3b_show.ps1`, the staging scripts |
| **MAME** | where the port has run: every result, so far, without exception |
| **live** | an interactive MAME session with video — what an eye gate is |
| **hardware** | a real CoCo 3. ★★★★★ **Unclaimed. Do not use this word about a result.** |

★★★★ **A tier-2 claim [§2.1] — "measured on the machine" — means MEASURED UNDER MAME and should say
so.** ★★★ The clock figure of 1.789772 MHz, the `$FFF8 → $FEF7 → $010C` vector chain and the stack
low-water of `S=$07C6` are all MAME measurements.

★★★★★ **Why this is a spec-level rule rather than an editing note: `live-disk` is the delivery gate
[§10], and the first genuine hardware run has to be distinguishable from everything written before
it.** ★★ If "on the machine" has silently meant MAME for forty tasks, the first hardware result reads
exactly like the previous forty.
...
**One AGI PICTURE (KQ1 #80) interpreted by 6809 code under MAME, 320×200×16.** Both
...
### 8A.4 ★★ What does not exist yet — ★★★★ SUPERSEDED IN PART, 2026-09-13, by §8B

> ★★★★ **Keyboard input, text rendering AND the on-screen input line now all exist** [§8B]. **A line
> typed on the keyboard reaches `said()` and the room answers it** [§8B.7]. What does not exist is
> the `>` prefix, the status line, and a key queue.
...
## 8B. ★★★★★ The text subsystem

★★★★★ **A line typed on the keyboard reaches `said()` and the room answers it** [P6.37]. Jay, live:
***"i did get a message box that it couldn't understand what i typed"*** — keys through the matrix,
editor, ENTER, parse, `said()`, the game's own reply.

★★★ **Four of the defects in this subsystem were found by a person watching a screen and NONE by a
byte gate** — see §8B.9, which is the more useful half of this section.

### 8B.1 ★★★ The opcodes

**Nine wired:** `$65 print`, `$66 print.v`, `$67 display`, `$68 display.v`, `$69 clear.lines`,
`$6C set.cursor.char`, `$6D set.text.attribute`, `$70 status.line.on`, `$71 status.line.off`.
`print`/`print.v` and `display`/`display.v` are implemented; the other five are minimal stubs.

★★★★ **`set.cursor.char` stays a stub ON EVIDENCE, not by omission.** `_inputCursorChar` is **0**
[`text.cpp:58`] and `inputEditOn`/`inputEditOff` are **both no-ops while it is zero** [`:673`, `:682`],
so the prompt renders completely without it. ★★ It also takes a **message number**, not a character
[`op_cmd.cpp:2106-2112`] — the opcode's name invites the wrong reading.

### 8B.2 ★★★★★ The text window is a PHASE, and it is four slots

★★★★★ **`$6000-$DFFF`, 32,768 B, slots 3-6, rows 0-24** — `TXT_ROWS`. A character row is 8 × 160 =
1,280 B; row 24's last byte is 31,999, and row 25's would be 33,279, **past the window and into slot
7's parser.**

★★★★★ **The mapping lives in `mmu_phase.s`, which is the SOLE writer of `$FFA0-$FFA7`** [§2N]. It got
there because three slots reached only row 18 and the command line is at row 22: `txt_blit` refuses a
glyph that would cross the end of the window, so **every prompt character was silently declined.**

> ★★★★ **The contract's exception, in that file's own header: slots 0-4 are no longer "never touched"
> — slots 3 and 4 are borrowed by `phase_text_in`/`_out` for one blit or one line clear and put back
> BY KNOWN VALUE.** The MMU registers are write-only, so there is no save/restore; ★★★ **those values
> are the only record of what was mapped, which is why they belong beside the contract they except.**

★★★★★ **Substitution runs BEFORE the window opens, and that is now a REQUIREMENT rather than luck.**
The message lives in the arena (slots 3-4) and the blit reads only `txt_pbuf`, in `MAP_INPUT` at
`$1C00` — **slot 0, resident in every phase.** ★★★ At three slots the arena was partly mapped and the
isolation was recorded as *"luck rather than design"*; **at four slots the arena is wholly unmapped
for the whole blit and there is no partial case left.**

★★★ **Nothing in slots 3-6 is read during a blit:** the arena is read by `txt_printf`, before the
window opens; the vocabulary (slot 5) by `par_parse`, which the oracle's parse-then-redraw order puts
first; the volume window (slot 6) by `res_open`, which is not on this path.

### 8B.3 ★★★★★ LOGIC decryption is bound to the BIND, not to the fetch

> ★★★★★ **`res_open` returns Sierra's raw bytes to every caller. `vm_bind_logic` decrypts on a fresh
> open only, signalled by carry.**

**Following the oracle, not our preference.** `agi.cpp:489-500` calls `decodeLogic()` **guarded by
`~flags & RES_LOADED`** — a cached logic skips it — and `logic.cpp:56` is the decrypt. ★★★★ **A
tree-wide search for `decrypt(` finds three call sites, NONE in the loader.**

★★★ **Authority: tier 3, ScummVM's structure** [§2.1] — ★★★★ **with the once-per-load part separated
out as a DATA-FORMAT constraint: the decrypt is in place, so running it twice re-encrypts.**

★★★★ **The alternative — decoding inside the fetch — breaks the `res` gate**, 1,264/1,264 → 959/1,264,
the mismatches being exactly the LOGIC subset. ★★★ **And the seam is OURS, chosen four phases ago:**
P1.1's oracle dump is taken immediately after `loadVolumeResource()` and **before any decode**, which
is why `tools/volread/`'s reference is raw.

### 8B.4 ★★ The hit/miss signal is CARRY, and that is forced

`res_open`'s hit path sets C; the miss path's `clr res_err` clears it free; `vm_bind_logic` tests
`bcs`. ★★★ **Carry rather than Z is forced: the caller's `lda res_err` destroys Z and N, and `lda`
does not touch C.** Cost: **7 emitted bytes** against ~17 for a memory flag.

> ★★★ **STANDING RISK: a returned CPU flag across a `jsr` is a contract NO TOOL ENFORCES.** Asserted
> in comments at both ends, fragile to any edit inserting a C-clobbering instruction between the call
> and the test.

### 8B.5 ★★★★ The message box: three rectangles, no save-under

★★★ **`tx_drawbox` follows `graphics.cpp:1079` / `text.cpp:507` including its colours** — white
background, red lines, 15 and 4, which are `$FF` and `$44` at 2 px/byte. ★★ **Three rectangles rather
than the oracle's five and byte-for-byte equivalent:** fill solid, hollow it, and the four inset lines
survive.

★★★★ **The window offset is applied AT DRAW TIME** [`graphics.cpp:1089`], not stored — `txt_bgy` is
game-screen and glyphs render at `txt_srow + txt_rowmin`. ★★★ Matching the oracle's structure here is
what kept the renderer gate's 4,594-rectangle comparison intact.

★★★★★ **Closing needs no saved pixels.** `text.cpp:560-564`: the window is closed by **re-rendering a
rectangle of the game screen into the display screen**; there is no save-under buffer anywhere. **Our
game screen is the shadow plane, our display screen the visible plane.** ★★★ Guarded by
**`txt_winactive`** — the oracle's own guard at `text.cpp:550` — **without which `txt_close` would
restore whatever rectangle the last box left behind.**

★★★ **The restore is scoped to the box's rectangle, NOT a whole-plane re-present.** ~1,900 bytes
copied instead of 26,880, ~6x cheaper — and ★★★★ **whole-plane re-presentation is rejected on
correctness, not cost: the compositor draws sprites onto the VISIBLE plane, so re-presenting all of it
erases every sprite until the next cycle** — a flicker on every dismissal in every room with an ego.

★★★★ **`charAttrib_Push`/`Pop` bracket the box** [`text.cpp:453`, `:455`, `:516`]. Porting the set
alone left black-on-white leaking permanently. ★★ **One level of save, not the oracle's stack** — its
`_textAttribArray` exists because it nests and our box does not; **the depth is stated rather than
assumed** [§2V.2]. The attribute's default is `charAttrib_Set(15, 0)` at construction
[`text.cpp:46`] and must be set, or the prompt row starts white.

### 8B.6 ★★★★★ `print` BLOCKS IN PLACE, and two game-visible governors control it

**The wait sits inside `vmop_print`, where the oracle's does.** ★★★★★ **The port has no continuation
mechanism** — suspending across `jsr ,x` would mean unwinding the 6809 return stack, and re-entering
`vm_interpret_cycle` calls **`res_cache_flush` with `res_depth` non-zero**, the defect that once
overwrote a running logic and halted all nine titles at cycle 0.

> ★★★★ **A design that never unwinds cannot move a cycle boundary, so the per-cycle diff the `vm`
> gate rests on is preserved BY CONSTRUCTION.**

**The two governors are the game's, not the harness's:**

- ★★★★ **`VM_FLAG_OUTPUT_MODE` (flag 15)** — set, and `messageBox` clears it, signals, and **returns
  without waiting** [`text.cpp:373-380`]. Checked **above** the loop.
- ★★★★ **`VM_VAR_WINDOW_AUTO_CLOSE_TIMER` (var 21)** — non-zero, and the wait auto-closes after N
  half-seconds [`text.cpp:386-392`], **and is zeroed on exit** [`:411`].

★★★★ **ENTER and ESCAPE dismiss; ESCAPE also cancels; every other key does nothing**
[`text.cpp:421-443`]. ★★ **This differs from `waitAnyKeyOrFinishedSound`, which breaks on ANY key**
[`keyboard.cpp:685-694`] — ★★★ **two waits, two rules, and one shared helper would silently give one
of them the other's semantics.**

★★★★★ **The wait's tick is vertical sync — 16.667 ms, so 30 ticks per half-second unit.** ScummVM's
`* 20` is 0.5 s over its own 25 ms unit; **twenty and thirty are the same half-second.** ★★★★ **The
loop must service what the cycle would have** — `text.cpp:398-399` calls `processAGIEvents()` and
`inGameTimerUpdate()` every pass — **and var 21 depends on the timer it ticks, so a loop that forgets
it never closes the box.**

### 8B.7 ★★★★★ The command line: per-cycle, not blocking

★★★★★ **`promptKeyPress` (`text.cpp:720`) is polled inside the main cycle's key dispatch**
[`cycle.cpp:350`] — **the opposite of `messageBox`**, which blocks in a nested inner loop.

★★★ **It is NOT `get.string`'s editor.** `stringKeyPress` (`text.cpp:987`) is a different routine for
a different feature, and `get.string` (opcode 73) remains unwired. ★★ **A component that does
something similar is not the component.**

**Behaviour, from the oracle:**

- **Keys `$20`-`$7F`, `$08` backspace, `$0D` ENTER; `$0A` explicitly ignored** [`text.cpp:740-789`].
  ★★ ESC is not handled here at all. ★★★ The printable range is **a ScummVM normalisation** — its own
  comment says *"Sierra didn't check for valid characters"*.
- ★★★★★ **ENTER parses IN PLACE** [`text.cpp:789`]: `parseUsingDictionary(&_prompt)`, then the cursor
  and buffer reset, then `promptRedraw()`. **Parse first, redraw second** — ★★★ which also keeps the
  vocabulary window and the text window disjoint, since **both want slot 5.**
- **The length bound is `VM_VAR_MAX_INPUT_CHARACTERS`** [`text.cpp:753-763`]; ★★ **at the limit the
  key is dropped silently** — no beep, no truncation, and indistinguishable from a dead keyboard to
  anyone watching.
- ★★★★★ **The prompt row is 22, not 0.** `text.cpp:63` initialises `_promptRow` to 0 and
  **`cycle.cpp:442` sets it to 22 in `runGame()`**, before the main loop, with no game asking.
  ★★★ **Reading only the declaration puts the command line at the top of the screen.**

★★★ **`accept.input` / `prevent.input` are a boolean AND a redraw** [`op_cmd.cpp:1985-1997`] —
`prevent.input` **clears the prompt row**, so it is not merely a flag.

★★★★ **`parseUsingDictionary` itself sets `VM_FLAG_ENTERED_CLI` (2) and clears
`VM_FLAG_SAID_ACCEPTED_INPUT` (4)** — **the two guards `testSaid` checks are established by the same
call a keystroke makes** — and `vm_post_cycle` clears both every cycle, matching `cycle.cpp:275`.

★★★ **The `>` prefix does not appear, and that is correct for now:** `promptRedraw` draws
`getString(0)` first, `vmop_set_string` is a bare `rts`, so `txt_strbase` is 0 and **the port skips
rather than reading from address 0.**

★★★★★ **The parse must run inside the vocabulary window.** The dictionary is paged in for the call
and out after; **a parse outside it walks the object table as a dictionary**, and the failure is a
hang rather than a wrong answer. ★★★ The port keeps **one** `par_parse` call site so there is nothing
for a second caller to get wrong.

### 8B.8 ★★★★★ The wait depends on an interrupt, and the vector chain has TWO hops

★★★★★ **Nothing in the text subsystem works without `hal_frame` advancing**, and for three tasks it
did not: no probe installed `$010C` or enabled GIME VBORD, so var 21's deadline — correctly computed
— could never arrive.

**The CoCo 3 chain, measured under MAME at the DECB prompt:** `$FFF8 → $FEF7` holds `16 02 12`
(`LBRA`, wrapping to `$010C`); `$010C` holds `7E D8 AF`, a `JMP` into DECB's handler.
★★★★ **`HAL_time_init` patches the SECOND hop, which is correct while `$FExx` is still ROM — and this
port's MMU remap destroys the FIRST.**

★★★ **Read again with IRQs masked to tell a remap from a crash:** a crash with `S=$F41C` pushes
through high memory and could have eaten `$FEF7` itself; **the two hypotheses predict the same
corpse.**

★★★★★ **Placement is a safety argument: enable AFTER calibration.** An IRQ inside `p3_cal` adds
cycles it cannot see and would report *"SLOW CLOCK"* about a build that has the fast clock —
★★★★ **corrupting a measurement rather than crashing, which is worse.**

### 8B.9 ★★★★★ What the gates here CANNOT see

> ★★★★★ **The text gate matched the oracle on 4,594 rectangles throughout the entire period in which
> nothing drew a box at all.**

★★★★★ **Four defects in this subsystem — no draw call, a 16-pixel offset, a stub restore, and
restore residue — were all found by a person watching a screen, and NONE was visible to any byte
gate.** ★★★★ **The gate's scope was its geometry; the defect was its absence.** Three colour defects
went the same way later.

★★★ **So every row here states what it cannot see:**

- **`p3b_text` / `p3b_box`** — geometry and completion. ★★★★ **They stage NO dictionary**, so they say
  nothing whatever about the parser, and did not from P6.4 until P6.36.
- **`p3b_parse`** — the parser, adjudicated on **word count**, because a broken vocabulary window
  parses to zero words and the run finishes normally. ★★ Gated on one title's 3,144-byte dictionary;
  **a full window is unexercised.**
- **`p3b_row22`** — ink on the command line. ★★★★ **Blind to COLOUR: all three colour defects would
  pass it.**
- **`p3b_type`** — `purpose=blocked`. ★★★★ **`natkeyboard` cannot hold a key long enough:** a finger
  holds one for tens of frames, `natkeyboard` for two or three, and the port looks once per cycle,
  twelve frames apart. **Zero of 180 posts delivered; a human's keypress arrives.**
- **Fault arms:** `p3b_notick` (the game clock starves), `p3b_nomap` (parse outside the window),
  `p3b_win3` (the window is three slots and the echo vanishes). ★★★ **All three red on the shipped
  binaries; a fault arm that has never been seen red is an unexercised assertion** [§2W].

### 8B.10 ★★ What still does not exist

**`set.string`**, and therefore the `>` prefix · **the status line**, which row 22's reachability now
unblocks · **a key queue or VBL latch**, without which a fast typist is dropped and `p3b_type` cannot
become a real gate · **`get.string`** (73), **`word.to.string`** (74), **`parse`** (75),
**`get.num`** (76) · and three position tests — **`obj.in.box`**, **`center.posn`**,
**`right.posn`** — which ordinary room logic uses.
...
**P3 — the display.** GIME mode, palette table, first room **under MAME**. **First eye gate. First HAL
contact** — §1.4's sync entry and §1.4.4's inherited-fix review happen here, not before.

★★ **P3b — the poke milestone.** A hardcoded room plus one LOGIC script running **under MAME**:
