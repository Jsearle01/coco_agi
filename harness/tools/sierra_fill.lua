-- harness/tools/sierra_fill.lua -- T-P0-020 / P3.11: FIND SIERRA'S ACTUAL FILL.
--
-- ★★★ P3.10 read $E1B1 and it is a BLIT, not a fill: it copies a pre-rendered buffer to
-- the screen through a 16-entry table, 24 cy/px, zero boundary tests. Its source pointer is
-- bounded by CMPX #$6000, so the picture is INTERPRETED into a buffer below $6000 -- and
-- the write census says exactly that: 0.69 to 1.56 picture-equivalents (26,880 B) are
-- written below $6000 during each DISK phase. The interpretation overlaps the load, which
-- is why six tasks could never separate disk time from render time.
--
-- ★★★ THIS HARNESS AIMS THE SAME METHOD AT THAT BUFFER, WITH ONE THRESHOLD DELIBERATELY
-- CHANGED: the run trigger is 16 bytes, not 128. A FLOOD FILL NEED NOT PRODUCE LONG RUNS --
-- our own span lengths are median 9 [T-P0-014] -- so reusing the blit's 128-byte trigger
-- would be blind to the very thing being hunted. [L-50: a property that discriminated one
-- routine is not automatically discriminating for another.]
--
...
local OUT   = os.getenv("SIERRA_OUT")   or "build/sierra_fill"
...
csv:write("frame,time_s,changed,fdc,voffset,total,"
       ...
       .. "w_seq,w_asc,w_other,w_maxrun,w_lo,w_hi,scr_w,unres,pcsamp,tracing,lowc\n")
local function w(f, ...)
...
local PC_AT_RUN = tonumber(os.getenv("SIERRA_RUN") or "16")  -- ★ L-44: stated, and
-- deliberately LOW: the fill is not assumed to emit long runs.
local LOW_TOP   = 0x6000   -- the blit's own source bound, CMPX #$6000
...
_G._low = 0                          -- ★ writes into the picture buffer, CPU $0000-$5FFF.
-- ★★ This initialiser was MISSING and the failure was instructive: Lua counts the format
-- string as argument #1, so `bad argument #41` for a 40-specifier format means the LAST
-- value was nil -- not that a 41st was wanted. I spent three checks re-counting specifiers
-- against arguments, which matched, before reading the index correctly.
_G._pc, _G._pcn = {}, 0
...
_G._t3 = prog:install_write_tap(0x0000, 0xFEFF, "all", function(off)
    ...
    if off < LOW_TOP then _G._low = _G._low + 1 end
    ...
    if off == _G._prev + 1 then
        ...
        if _G._run == PC_AT_RUN then
            ...
            -- ★★★ THE TRIGGER: a run landing in the PICTURE BUFFER, CPU $0000-$5FFF.
            ...
            if off < LOW_TOP then _G._saw_run = true end
        end
...
local function trace_start(f, t, acc)
    ...
    _G._win_maxrun, _G._win_scrw, _G._win_low = 0, 0, 0
...
local function trace_stop(f, t)
    ...
    if (_G._win_low or 0) > 2000 then
        w("           ★★★ THIS WINDOW WRITES THE PICTURE BUFFER (%d writes below $6000).",
          _G._win_low or 0)
    else
        w("           (too few writes below $6000 -- %s)",
...
w("sierra_fill: gate = graphics mode + a %d+ byte run landing BELOW $6000 --", PC_AT_RUN)
w("             the buffer the blit reads. Threshold is LOW on purpose: a flood fill")
w("             need not emit long runs.")
...
_G._n = emu.add_machine_frame_notifier(function()
    ...
    local lowc = _G._low
    _G._scrw, _G._unres, _G._low = 0, 0, 0
    ...
    if tracing then
        ...
        _G._win_low    = (_G._win_low or 0) + lowc
    end
    ...
    -- ★ fires DURING the disk burst: that is when the sub-$6000 writes happen.
    -- ★★★ RESTORED-STATE MODE. The trackers are built from WRITE TAPS, and a restored state
    -- has already done those writes: VMODE and VOFFSET are never rewritten, so the graphics-mode
    -- condition can never become true and the gate would never arm. [Jay confirmed the restore
    -- reaches gameplay by watching it; my verification script produced an empty file, which was
    -- a defect in the script, not in the state.]
    -- ★ SIERRA_ASSUME_INGAME=1 asserts what the operator observed. It is an ASSERTION, not a
    -- measurement, and it is only sound when launching with -state.
    -- ★★ The sub-$6000 trigger itself needs no physical resolution -- it is a CPU address -- so
    -- this affects only the mode condition.
    local INGAME = os.getenv("SIERRA_ASSUME_INGAME") == "1"
    if armed and not tracing and _G._saw_run
       and (INGAME or (_G._vmode >= 0 and (_G._vmode & 0x80) ~= 0)) then
    ...
    csv:write(string.format(
        "%d,%.9f,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\n",
        ...
        sw, ur, pn, tracing and 1 or 0, lowc))
end)
