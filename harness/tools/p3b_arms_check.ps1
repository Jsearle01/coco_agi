...
$ARMS = @(
  ...
  # ★★★★ RE-BASELINED T-P0-137, ALL NINE BY EXACTLY +8 B. res_cnext separates the two reasons its
  # slow path used to serve: a CROSSING re-derives the address, a THEFT of slot 6 only re-maps the
  # block it already knows. **Theft is the common case in the compositor** -- every row is decoded
  # through slot 6 and then composited through it -- and the castle crosses no boundary at all, so
  # before this change essentially every slow-path entry paid a crossing's price for a theft.
  # ★★★ Pixels identical (co_WRITTEN 11,360 and CP_BLITS 128 against the -SlowSteal before arm);
  # steady-window mean 0.2968 -> 0.2917 s/cycle, median 0.3004 -> 0.2837.
  # RETIRED at T-P0-137: p3b 15,875 FB06064E | p3b_text 17,033 7E4AEBEC | p3b_win3 17,033 858EC6CD
  #   p3b_notick 17,030 B85CA94F | p3b_nomap 17,030 B02F48D2 | p3b_fault 16,079 D3ED625F
  #   p3b_flat 16,064 A5FBBCDD | p3b_comb 19,097 793E831B | p3b_comb_count 19,132 AA7E8B29
  # ★★★★★ RE-BASELINED T-P0-138, ALL NINE SHRINKING BY EXACTLY 142 B: p3b_probe.s now defines
  # PIC_NOCOUNT, so pic_fill.s's diagnostic counters are not assembled. **fc_count was 29.4% of the
  # room-render cycle** -- a `ldd CNT_CHK+2 / addd #1 / std` in the fill's innermost loops -- and
  # nothing in this probe or any host reads those bytes. pic_fill.s's own comment has said since it
  # was written that "timings come from -DPIC_NOCOUNT, where this vanishes entirely".
  # ★★★★ Room change 9.5456 -> 6.1579 s; the render inside it 8.2797 -> 4.8957 s. **The steady
  # cycle is unchanged** (mean 0.2917, median 0.2837 both arms) because the fill runs once a room.
  # ★★★ Pixels identical: co_WRITTEN 11,360 and CP_BLITS 128 against -DP3B_PIC_COUNT.
  # ★★ The 142 B also give region A back some of what T-P0-136 took [P6.84 §1.2(2)].
  # RETIRED at T-P0-138: p3b 15,883 CF600CD0 | p3b_text 17,041 817747AC | p3b_win3 17,041 38BCE1D9
  #   p3b_notick 17,038 F28109AA | p3b_nomap 17,038 81A538DC | p3b_fault 16,087 87FF3CFB
  #   p3b_flat 16,072 66D4338B | p3b_comb 19,105 57CCB100 | p3b_comb_count 19,140 92C3ADC4
  # ★★★★★ RE-BASELINED T-P0-140, THE THREE CEL-LINKED ARMS ONLY, +2 B EACH. composite.s's priority
  # WRITE reuses the address its priority READ already formed for the same pixel, instead of
  # calling plane_pri a second time. **The six text arms link no compositor and are byte-identical**
  # -- verified, not assumed, and that split is the check that the change is scoped.
  # ★★★★ co_rej_pri is ZERO in the castle, so every opaque pixel is drawn and exactly half of all
  # plane_pri calls were the re-derivation: 11,360 of 22,720 over 40 cycles, plane_pri being 11.5%
  # of the drawing stage. ★★★ Steady cycle mean 0.2925 -> 0.2791, median 0.2837 -> 0.2670.
  ...
  # ★★★★★ RE-BASELINED T-P0-149: ALL NINE moved, +176 B (+178 on the three cel arms, which also
  # carry pca_lp's `leay` over the new seventh staged byte). ★★★★ The cause is UNCONDITIONAL and
  # that is why every arm moved: P3_SPR_SIZE went 6 -> 7 to hold the oracle's sortOrder, and
  # p3_stage_sprites now SORTS [sprite.cpp:63]. **Not a flag; a fidelity fix the port had been
  # missing since staging was written.**
  # ★★ RETIRED at T-P0-149: p3b 16,114 8548D2A5 | p3b_text 16,899 D1D65E0A | p3b_win3 16,899
  # 4A183DF1 | p3b_notick 16,896 841BC1AD | p3b_nomap 16,896 2A298702 | p3b_fault 15,945 4A31BB0D
  # | p3b_flat 15,930 18B0E2FB | p3b_comb 19,336 B5A60E1D | p3b_comb_count 19,371 A3DD030C
  # ★★★★★ RE-BASELINED T-P0-152: ONLY THE THREE CEL ARMS moved, +39 B each -- p3_restore_prev and
  # prp_copy are inside `ifdef P3B_CEL_LINK`, so the six text arms are untouched. ★★★ That split is
  # itself the check: a change to the restore walk that moved a text arm would mean it had reached
  ...
  @{ n = "p3b_text";   f = $TEXT;                                         sz = 17075; sha = "80527351" },
  @{ n = "p3b_win3";   f = $TEXT + @("-DTEXT_WIN3");                      sz = 17075; sha = "5CDE2845" },
  @{ n = "p3b_notick"; f = $TEXT + @("-DTEXT_FAULT_NOTICK");              sz = 17072; sha = "06B965E2" },
  @{ n = "p3b_nomap";  f = $TEXT + @("-DP3B_VOCAB_NOMAP");                sz = 17072; sha = "E5F90993" },
  @{ n = "p3b_fault";  f = $TEXT + @("-DTEXT_MODELLED");                  sz = 16121; sha = "B56E36AD" },
  @{ n = "p3b_flat";   f = $TEXT + @("-DTEXT_MODELLED","-DTEXT_VOCAB_FLAT"); sz = 16106; sha = "78CFE4B7" },
